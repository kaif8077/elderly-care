...
const AdminUsers = () => {
  ...
  const columns = [
      ...
      render: (_, user) => (
        <Space>
          ...
          <div>
            ...
            <br />
            <Text type="secondary">{user.elderlyCareId || 'ID pending'}</Text>
          </div>
        </Space>
      )
    },
    ...
    },
    {
      ...
      width: 150,
      render: (value) => <Progress percent={value || 0} size="small" strokeColor="#0066ff" />
    },
    {
      title: 'QR',
      dataIndex: 'qrStatus',
      key: 'qrStatus',
      render: (value) => <Tag color={tagColor(value)}>{value || 'missing'}</Tag>
    },
    {
      title: 'Registered',
      dataIndex: 'createdAt',
        ...
        <Row gutter={[12, 12]} align="middle">
          ...
          <Col xs={24} sm={12} lg={4}>
            <Select
              ...
              onChange={(value) => updateQuery({ profileStatus: value })}
              placeholder="Profile status"
              style={{ width: '100%' }}
              options={['complete', 'incomplete'].map((value) => ({ value, label: value }))}
            />
          </Col>
          <Col xs={24} sm={12} lg={4}>
            <Select
              ...
              onChange={(value) => updateQuery({ bloodGroup: value })}
          ...
          <Col xs={24} sm={12} lg={4}>
            <Select
              size="middle"
              allowClear
              value={query.qrStatus}
              onChange={(value) => updateQuery({ qrStatus: value })}
              placeholder="QR status"
              style={{ width: '100%' }}
              options={['active', 'revoked', 'generated', 'missing'].map((value) => ({
                value,
                label: value
              }))}
            />
        ...
        {screens.md ? (
          ...
          <>
              ...
              renderItem={(user) => (
                <List.Item>
                  <article className="admin-user-mobile-card">
                    ...
                    <div className="admin-user-mobile-progress">
            ...
            {data.pagination.total > data.pagination.limit && (
              <Pagination
                className="admin-users-mobile-pagination"
                simple
                current={data.pagination.page}
                pageSize={data.pagination.limit}
                total={data.pagination.total}
