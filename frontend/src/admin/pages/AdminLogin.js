...
const AdminLogin = () => {
  ...
  const submit = async (values) => {
    ...
    try {
      await login(values);
      navigate(
        location.state?.from?.startsWith('/admin/') ? location.state.from : '/admin/dashboard',
        { replace: true }
      );
    } catch (requestError) {
      ...
      <main className="care-auth-page">
        ...
        <Card className="care-auth-card" styles={{ body: { padding: 34 } }}>
          <div className="care-auth-brand">
            ...
            <div>
              <Text className="care-eyebrow">ELDERLYCARE ADMINISTRATION</Text>
              <br />
              <Text type="secondary">Restricted management workspace</Text>
            </div>
          </div>
          <Title level={2}>Admin sign in</Title>
          <Text type="secondary">Use an authorized administrator account to continue.</Text>
          ...
          <Form layout="vertical" onFinish={submit} style={{ marginTop: 24 }}>
            ...
            <Button size="middle" type="primary" block htmlType="submit" loading={loading}>
              Sign in securely
            </Button>
          </Form>
          <Button type="link" block onClick={() => navigate('/')}>
            Return to ElderlyCare
          </Button>
        </Card>
      </main>
    </ConfigProvider>
