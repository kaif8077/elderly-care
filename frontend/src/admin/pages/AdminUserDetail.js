...
const dateValue = (date, includeTime = false) =>
  date
    ? new Date(date).toLocaleString(
        undefined,
        includeTime ? undefined : { year: 'numeric', month: 'short', day: 'numeric' }
      )
    : 'Not available';
...
const AdminUserDetail = () => {
  ...
  if (loading)
    return <div className="admin-detail-skeleton skeleton" aria-label="Loading user details" />;
  if (error)
    return (
  ...
  const archiveAccount = async (event) => {
    ...
    try {
      await adminApi.delete(`/users/${userId}`, {
        data: { reason, confirmation: archiveForm.confirmation }
      });
  ...
  const restoreAccount = async () => {
    if (
      !window.confirm(
        'Restore this archived account? QR access will remain revoked until a new code is generated.'
      )
    )
      return;
    setStatusSaving(true);
  ...
  const overview = (
    <div className="admin-detail-grid">
      ...
      <section className="admin-detail-card">
        ...
        <dl>
          <Field label="Medical profile">
            <AdminStatusBadge status={user.profileStatus} />
          </Field>
          <Field label="Completion">{user.profileCompletion}%</Field>
          <Field label="QR record">
            <AdminStatusBadge status={qr.status} />
          </Field>
        </dl>
        <div className="admin-detail-actions">
          <Link to={`/admin/id-cards/${userId}`}>
            <Button type="primary" icon={<FaIdCard />}>
              View ID card
            </Button>
          </Link>
          {user.isDeleted && (
            <Button type="primary" loading={statusSaving} onClick={restoreAccount}>
              Restore account
            </Button>
          )}
          {!user.isDeleted && (
            <Button danger disabled={statusSaving} onClick={() => setArchiveOpen(true)}>
              Archive user
            </Button>
          )}
        </div>
  ...
  const sections = {
    ...
    Personal: profile ? (
      <section className="admin-detail-card">
        ...
        <dl className="admin-detail-columns">
          <Field label="Full name">{value(personal.name)}</Field>
          <Field label="Date of birth">{dateValue(personal.dob)}</Field>
          <Field label="Gender">{value(personal.gender)}</Field>
          <Field label="Blood group">{value(personal.bloodGroup)}</Field>
          ...
          <Field label="Preferred language">
            {listValue(personal.preferredLanguage, personal.otherLanguage)}
          </Field>
    ...
    Contact: profile ? (
      <div className="admin-detail-grid">
        <section className="admin-detail-card">
          ...
          <dl>
            <Field label="Phone">{value(contact.phone)}</Field>
            <Field label="Address">{value(contact.address)}</Field>
          </dl>
        </section>
        <section className="admin-detail-card">
          <h2>Emergency contact</h2>
          <dl>
            <Field label="Name">{value(emergency.name)}</Field>
            <Field label="Phone">{value(emergency.phone)}</Field>
    ...
    Medical: profile ? (
      <section className="admin-detail-card admin-sensitive-card">
        <div className="admin-section-warning">
          Sensitive medical information — access is audit logged.
        </div>
        <h2>Medical information</h2>
        <dl>
          <Field label="Medical history">
            {listValue(medical.medicalHistory, medical.medicalHistoryOther)}
          </Field>
          <Field label="Allergies">{listValue(medical.allergies, medical.allergiesOther)}</Field>
          <Field label="Medications">
            {listValue(medical.medications, medical.medicationsOther)}
          </Field>
          <Field label="Current symptoms">
            {listValue(medical.currentSymptoms, medical.currentSymptomsOther)}
          </Field>
        </dl>
      </section>
    ) : null,
    Insurance: profile ? (
      <section className="admin-detail-card admin-sensitive-card">
        <div className="admin-section-warning">Private insurance information</div>
        <h2>Insurance</h2>
        <dl>
    ...
    QR: (
      <section className="admin-detail-card">
        ...
        <dl>
          ...
          <Field label="Revocation support">
            {qr.revocationSupported ? 'Available' : 'Not available in legacy model'}
          </Field>
        </dl>
        <p className="admin-privacy-note">
          <FaQrcode aria-hidden="true" /> QR payload data is intentionally not displayed in the
          admin directory.
        </p>
      </section>
    )
  ...
  return (
    <div className="admin-user-detail-page">
      ...
      <header className="admin-detail-hero">
        ...
        <div>
          <p className="admin-eyebrow">User record</p>
          <h1>{user.name}</h1>
          <p>{user.email}</p>
        </div>
      ...
      <Space wrap role="tablist" aria-label="User record sections" style={{ margin: '18px 0' }}>
        {tabs.map((tab) => (
          <Button
            ...
            id={`admin-tab-${tab}`}
            role="tab"
            aria-controls="admin-user-tabpanel"
      ...
      <div id="admin-user-tabpanel" role="tabpanel" aria-labelledby={`admin-tab-${activeTab}`}>
        {!profile && !['Overview', 'QR'].includes(activeTab) ? (
          <div className="admin-state-card">
            ...
            <p>This user has not submitted medical information.</p>
          </div>
        ) : (
          sections[activeTab]
        )}
      ...
      {archiveOpen && (
        <div className="admin-modal-backdrop" role="presentation">
          ...
          >
            ...
            <p>
              This blocks login, revokes active sessions and QR codes, and hides the account from
              active-user lists. Medical records are preserved.
            </p>
            <form onSubmit={archiveAccount}>
              <label>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setArchiveForm((current) => ({ ...current, reason: event.target.value }))
                  }
                >
              ...
              <label>
                ...
                <textarea
                  required={archiveForm.reason === 'Other'}
                  maxLength="500"
                  value={archiveForm.details}
                  onChange={(event) =>
                    setArchiveForm((current) => ({ ...current, details: event.target.value }))
                  }
                />
              </label>
              <label>
                ...
                <input
                  autoFocus
                  value={archiveForm.confirmation}
                  onChange={(event) =>
                    setArchiveForm((current) => ({ ...current, confirmation: event.target.value }))
                  }
                />
              </label>
              <div className="admin-modal-actions">
                <Button onClick={() => setArchiveOpen(false)} disabled={statusSaving}>
                  Cancel
                </Button>
                <Button
                  htmlType="submit"
                  ...
                  disabled={!archiveForm.reason || archiveForm.confirmation !== 'DELETE'}
                >
                  Archive user
                </Button>
              </div>
