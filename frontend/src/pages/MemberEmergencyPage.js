    ...
    try {
      ...
      setState((current) => ({
        ...
        error:
          ...
          'Complete your medical profile before creating the emergency ID card.'
      }));
    }
  };
  ...
  const regenerate = async () => {
    ...
    try {
      ...
      setState((current) => ({
        ...current,
        working: false,
        error: error.response?.data?.message || 'Unable to regenerate the emergency QR.'
      }));
    }
  };
    ...
    <main className="care-page member-focused-page">
      <div className="member-page-title">
        ...
        <Typography.Text type="secondary">
          Your current profile and QR are combined into a printable emergency card.
        </Typography.Text>
      </div>
      {state.error && (
        <Alert
      ...
      {!state.profile ? (
        <Empty description="Complete your medical profile to generate an ID card">
          <Button type="primary" href="/medical-profile">
            Complete profile
          </Button>
        </Empty>
      ) : (
        <Space direction="vertical" size={18} style={{ width: '100%' }}>
          <Card
            ...
            extra={
              <Button icon={<ReloadOutlined />} loading={state.working} onClick={regenerate}>
                Regenerate QR
              </Button>
            }
          >
            ...
            <Descriptions
              ...
              items={[
                { key: 'name', label: 'Name', children: primary?.name || 'Not provided' },
                {
                  key: 'relation',
                  label: 'Relationship',
                  children: primary?.relationship || 'Not provided'
                },
                {
                  key: 'phone',
                  label: 'Phone',
