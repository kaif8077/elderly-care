  ...
  return (
    <main className="care-auth-page"><AuthShowcase /><Card className="care-auth-card" styles={{ body: { padding: 32 } }}>
      ...
      {step === 0 ? <Form form={form} layout="vertical" requiredMark="optional" onFinish={requestOtp}>
        <Form.Item label="Full name" name="name" rules={[{ required: true, message: 'Enter your full name' }]}><Input prefix={<UserOutlined />} autoComplete="name" placeholder="Enter your full name" /></Form.Item>
        <Form.Item label="Email address" name="email" rules={[{ required: true }, { type: 'email' }]}><Input prefix={<MailOutlined />} autoComplete="email" placeholder="Enter your email address" /></Form.Item>
        <Form.Item label="Password" name="password" dependencies={['confirmPassword']} rules={[{ required: true }, { validator: (_, value) => strength(value) === 5 ? Promise.resolve() : Promise.reject(new Error('Use 8+ characters with upper/lowercase, number and special character')) }]}><Input.Password prefix={<LockOutlined />} autoComplete="new-password" placeholder="Enter a strong password" /></Form.Item>
        ...
        <Button block type="primary" htmlType="submit" loading={loading}>Send verification code</Button>
      </Form> : <Form layout="vertical" onFinish={verify}><Alert type="info" showIcon message={`A 6-digit code was sent to ${account.email}`} style={{ marginBottom: 20 }} /><Form.Item label="Verification code" name="otp" rules={[{ required: true }, { len: 6, message: 'Enter the 6-digit code' }]}><Input.OTP length={6} /></Form.Item><Button block type="primary" htmlType="submit" loading={loading}>Verify and open dashboard</Button><Button block type="link" onClick={resend} disabled={loading}>Resend code</Button></Form>}
