      ...
      try {
        ...
        if (medical.data.profilePhoto) {
          try {
            ...
            /* The fallback avatar remains available. */
          }
        }
      } catch (error) {
                  ...
                  items={[
                    {
                      ...
                      children: profile.hasInsurance ? 'Active' : 'Not active'
                    },
                    {
                      key: 'provider',
                      label: 'Provider',
                      children: show(profile.hasInsurance && profile.insuranceProvider)
                    },
                    {
                      key: 'policy',
                      label: 'Policy number',
                      children: show(profile.hasInsurance && profile.policyNumber)
                    }
                  ]}
                />
              )
  ...
  return (
    <main className="care-page member-profile-page">
      <Flex
        ...
        gap={16}
        wrap="wrap"
        className="member-page-title"
      >
        <div>
          <Title level={2}>My Profile</Title>
          <Text type="secondary">
            Review and update your personal, medical, and emergency information.
          </Text>
        </div>
            ...
            <Flex align="center" gap={18} className="member-profile-identity">
              <div className="member-profile-photo-edit">
                ...
                <Button
                  ...
                  shape="circle"
                  icon={<EditOutlined />}
                  aria-label="Edit profile photograph"
              ...
              <div>
                ...
                <Text type="secondary">
                  {age !== null ? `Age: ${age}` : `DOB: ${formatDate(profile.dob)}`} ·{' '}
                  {show(profile.gender)}
                </Text>
              </div>
            </Flex>
            ...
            <Descriptions
              ...
              items={[
                { key: 'member', label: 'ElderlyCare ID', children: show(profile.elderlyCareId) },
                {
                  key: 'blood',
                  label: 'Blood group',
                  children: <Tag color="orange">{show(profile.bloodGroup)}</Tag>
                },
                ...
                items={[
                  {
                    ...
                    children: show(profile.medicalHistory)
                  },
                  { key: 'allergies', label: 'Allergies', children: show(profile.allergies) },
                  {
                    key: 'medicines',
                    label: 'Current medications',
                    children: show(profile.medications)
                  }
                ]}
              />
            </Card>
