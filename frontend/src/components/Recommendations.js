...
import { Alert, Button, Card, Empty, Flex, List, Skeleton, Space, Tag, Typography } from 'antd';
import {
  DownloadOutlined,
  HeartOutlined,
  HistoryOutlined,
...
const Recommendations = ({ showPageHeader = false }) => {
  ...
  const sendFeedback = async (id, relevance) => {
    try {
      await axios.patch(
        `${base}/api/recommendations/health/${id}/feedback`,
        { relevance },
        { headers }
      );
      await load();
    } catch (requestError) {
      setError('Unable to save recommendation feedback.');
    }
  };
      ...
      {showPageHeader && (
        <Flex className="member-page-title" justify="space-between" align="center" gap={16} wrap>
          <div>
            ...
            <Text type="secondary">
              Generate, read, download, and review your personalized guidance.
            </Text>
          </div>
          <Button
            type="primary"
      ...
      <Card className="member-content-card">
        <Space align="start" style={{ display: 'flex', marginBottom: 18 }}>
          <div>
            ...
            <h6 className="care-secondary-heading" style={{ margin: '5px 0' }}>
              <HeartOutlined /> Necessary health guidance
            </h6>
            <Paragraph type="secondary" style={{ margin: 0 }}>
              A short list based on your latest medical profile.
            </Paragraph>
          </div>
        </Space>
        ...
        {loading ? (
            ...
            <Card
              ...
              title={
                <Space>
                  ...
                  <Text>{new Date(latest.generatedAt).toLocaleString()}</Text>
                </Space>
              }
              extra={
                <Button icon={<DownloadOutlined />} onClick={() => download(latest._id)}>
                  PDF
                </Button>
              }
            >
              ...
              {latest.sourceSummary && (
                <div style={{ marginTop: 16 }}>
                  <Text type="secondary">Generated from: </Text>
                  <Space wrap>
                    {[
                      ...(latest.sourceSummary.conditions || []),
                      ...(latest.sourceSummary.allergies || []),
                      ...(latest.sourceSummary.medications || []),
                      ...(latest.sourceSummary.symptoms || [])
                    ]
                      .slice(0, 8)
                      .map((value) => (
              ...
              <Space style={{ marginTop: 16 }}>
                <Text>Was this guidance relevant?</Text>
                <Button
                  type={latest.feedback?.relevance === 'helpful' ? 'primary' : 'default'}
                  icon={<LikeOutlined />}
                  onClick={() => sendFeedback(latest._id, 'helpful')}
                >
                  Helpful
                </Button>
                <Button
                  type={latest.feedback?.relevance === 'not_helpful' ? 'primary' : 'default'}
                  icon={<DislikeOutlined />}
                  onClick={() => sendFeedback(latest._id, 'not_helpful')}
                >
                  Not helpful
                </Button>
              </Space>
            ...
            >
              {items.length > 1 ? (
                ...
                <Empty
                  ...
                  description="No previous recommendations"
                />
              )}
            </Card>
          </div>
          ...
          <Empty description="Complete your medical profile, then generate your first recommendation.">
            <Button type="primary" onClick={generate} loading={working}>
