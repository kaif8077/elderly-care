...
// Creates a reusable Ant Design required-field validation rule.
const required = (message) => [{ required: true, message }];
const { useBreakpoint } = Grid;

// Manages the step-by-step medical profile form and saves each completed section.
const MedicalForm = ({ onSubmissionSuccess }) => {
  ...
  // Converts form values into the backend profile payload for a draft or final save.
  const serialize = (raw, finalize = false, fields = null) => {
    const { reviewConfirmed, profilePhoto, ...clean } = raw;
    const data = fields
      ? Object.fromEntries(
          fields.filter((field) => field !== 'profilePhoto').map((field) => [field, clean[field]])
        )
      : clean;
    return {
      ...data,
  ...
  // Uploads the selected profile photograph after the profile record exists.
  const savePhoto = async () => {
    ...
    setProfile((current) => ({
      ...(current || {}),
      profilePhoto: { uploadedAt: new Date().toISOString() }
    }));
  };

  // Validates and persists the current section before moving to another step.
  ...
  // Finalizes the reviewed medical profile and opens the completed profile page.
  const submit = async () => {
    setSaving(true);
    try {
      ...
      if (error.errorFields?.length) {
        ...
        const fields = Array.isArray(response?.fields)
          ...
          : response?.fields && typeof response.fields === 'object'
            ...
            : '';
        toast.error(
          `${response?.message || 'Unable to save the medical profile.'}${fields ? ` Missing or invalid: ${fields}.` : ''}`
        );
      }
    } finally {
      setSaving(false);
  ...
  // Keeps only the selected image file in local state for preview and upload.
  ...
  return (
    <section className="medical-form">
      <div className="medical-form-progress">
        <Text type="secondary">Step {step + 1} of 4 · Each step is saved before you continue.</Text>
      </div>
      ...
      >
        {step === 0 && (
          <Card size="small" className="medical-step-card">
            ...
            <Row gutter={16}>
              <Col xs={24} md={12}>
                <Form.Item name="firstName" label="First name" rules={required('Enter first name')}>
                  <Input placeholder="Enter first name" />
                </Form.Item>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item name="lastName" label="Last name" rules={required('Enter last name')}>
                  <Input placeholder="Enter last name" />
                </Form.Item>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item name="gender" label="Gender" rules={required('Select gender')}>
                  <Select
              ...
              <Col xs={24} md={12}>
                <Form.Item name="height" label="Height (cm)" rules={required('Enter height')}>
                  <InputNumber
              ...
              <Col xs={24} md={12}>
                <Form.Item name="weight" label="Weight (kg)" rules={required('Enter weight')}>
                  <InputNumber
              ...
              {values.preferredLanguage?.includes('Other') && (
                <Col span={24}>
                  <Form.Item
                    name="otherLanguage"
                    label="Other language"
                    rules={required('Enter the other language')}
                  >
                    <Input placeholder="Type another language" />
                  </Form.Item>
                </Col>
              )}
              <Col xs={24} md={12}>
                <Form.Item
                  name="mobilityStatus"
                  label="Mobility"
                  rules={required('Select mobility')}
                >
                  <Select
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  ...
                  rules={required('Select diet preference')}
                >
                  <Select
                    placeholder="Select diet preference"
                    options={['Vegetarian', 'Non-Vegetarian', 'Vegan', 'Eggetarian'].map(
              ...
              <Col xs={24} md={12}>
                ...
                >
                  <Select
                    ...
                    options={['single', 'married', 'widowed', 'divorced', 'separated'].map(
        ...
        {step === 1 && (
          <Card size="small" className="medical-step-card">
            ...
            <Alert
              showIcon
              type="info"
              message="Add at least one complete emergency contact. You can add more contacts when needed."
            ...
            <Row gutter={16}>
              <Col xs={24} md={8}>
                <Form.Item
                  ...
                  rules={required('Enter contact number')}
                >
              ...
              <Col xs={24} md={16}>
                <Form.Item
                  ...
                  rules={required('Enter residential address')}
                >
                  <Input placeholder="Enter complete residential address" />
                </Form.Item>
              </Col>
            </Row>
                      ...
                      <Row gutter={16}>
                        <Col xs={24} md={12}>
                          <Form.Item
                            ...
                            label="Contact name"
                            rules={required('Enter contact name')}
                          >
                        ...
                        <Col xs={24} md={12}>
                          <Form.Item
                            ...
                            label="Relationship"
                            rules={required('Enter relationship')}
                          >
                            <Input placeholder="e.g. Son or daughter" />
                          </Form.Item>
                        </Col>
                        <Col xs={24} md={12}>
                        ...
                        <Col xs={24} md={12}>
                          <Form.Item {...field} name={[field.name, 'priority']} label="Priority">
                        ...
                        <Col xs={24} md={12}>
                          <Form.Item
                            {...field}
                            name={[field.name, 'preferredChannel']}
                            label="Preferred channel"
                          >
                            <Select
                              options={['phone', 'email', 'sms', 'telegram'].map((value) => ({
                                value,
                                label: value
                              }))}
                            />
                        ...
                        <Col xs={24}>
                          <Space wrap className="medical-contact-permissions">
                            <Form.Item
                              {...field}
                            ...
                            <Form.Item
                              {...field}
                              name={[field.name, 'canReceiveAlerts']}
                              valuePropName="checked"
                            >
                              <Checkbox>Receive alerts</Checkbox>
                            </Form.Item>
                            <Form.Item
                              {...field}
                              name={[field.name, 'canAccessReports']}
                              valuePropName="checked"
                            >
                              <Checkbox>Access reports</Checkbox>
                            </Form.Item>
                            <Form.Item
                              {...field}
                              name={[field.name, 'canManageAccount']}
                              valuePropName="checked"
                            >
                              <Checkbox>Manage account</Checkbox>
                            </Form.Item>
                          </Space>
                        </Col>
                      </Row>
        ...
        {step === 2 && (
          <Card size="small" className="medical-step-card">
            ...
            <Paragraph type="secondary">
              Select an option or type a new value and press Enter.
            </Paragraph>
            <Row gutter={16}>
                ...
                <Col xs={24} md={12} key={field}>
                  <Form.Item name={field} label={labels[field]}>
                    <Select
                      mode="tags"
                      tokenSeparators={[',']}
                      placeholder={`Select or type ${labels[field].toLowerCase()}`}
              ...
              <Col xs={24} md={12}>
                <Form.Item name="doctorName" label="Treating doctor">
                  <Input placeholder="Enter treating doctor name" />
                </Form.Item>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item name="doctorPhone" label="Doctor phone">
                  <Input placeholder="Enter doctor phone number" />
                </Form.Item>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item name="preferredHospital" label="Preferred hospital">
                  <Input placeholder="Enter preferred hospital" />
                </Form.Item>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item name="fallRisk" label="Fall risk" valuePropName="checked">
            ...
            <Card size="small">
              <Row gutter={[18, 18]}>
                <Col xs={24} md={8}>
                  ...
                  <div>
                    <Text strong>{fullName || 'Not entered'}</Text>
                  </div>
                </Col>
                <Col xs={24} md={8}>
                  <Text type="secondary">Blood group</Text>
                  <div>
                    <Text strong>{values.bloodGroup || 'Not selected'}</Text>
                  </div>
                </Col>
                <Col xs={24} md={8}>
                  <Text type="secondary">Known conditions</Text>
                  <div>
                    <Text strong>{values.medicalHistory?.join(', ') || 'None entered'}</Text>
                  </div>
                </Col>
                <Col span={24}>
                  <Text type="secondary">Emergency contacts</Text>
                  <div>
                    <Text strong>
              ...
              rules={[
                {
                  validator: (_, checked) =>
                    checked
                      ? Promise.resolve()
                      : Promise.reject(new Error('Confirm the summary before saving'))
                }
              ]}
              style={{ marginTop: 18 }}
            >
        ...
        <Space style={{ display: 'flex', justifyContent: 'flex-end' }} wrap>
          {step > 0 && (
            <Button size="middle" onClick={() => setStep((value) => value - 1)}>
              Previous
            </Button>
          )}
          {step < 3 ? (
            ...
            <Button type="primary" size="middle" loading={saving} onClick={submit}>
