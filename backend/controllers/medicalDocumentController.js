...
exports.list = async (req, res) => {
  const documents = await MedicalDocument.find({ userId: req.user.id, status: 'active' })
...
exports.upload = async (req, res) => {
  if (!req.file)
    return res.status(400).json({ message: 'Choose a PDF or supported image up to 5 MB' });
  const detected = detectContentType(req.file.buffer);
  if (!detected || detected !== req.file.mimetype)
    return res
      .status(400)
      .json({ message: 'File content does not match an allowed document type' });
  if (!categories.includes(req.body.category))
    return res.status(400).json({ message: 'Choose a valid document category' });
  const displayName = String(req.body.displayName || req.file.originalname)
    .trim()
    .slice(0, 120);
  ...
  const profile = await MedicalProfile.findOne({ userId: req.user.id })
    ...
    .lean();
  if (!profile)
    return res.status(409).json({ message: 'Create a medical profile before uploading documents' });
  const storageFileId = await store({
  ...
  return res.status(201).json({
    ...
    document: { ...document.toObject(), storageFileId: undefined }
  });
};
exports.download = async (req, res) => {
  ...
  res.set({
    ...
    'Content-Disposition': `attachment; filename="${document.displayName.replace(/["\r\n]/g, '')}"`,
    'Cache-Control': 'private, no-store'
  });
  return bucket()
    .openDownloadStream(new mongoose.Types.ObjectId(document.storageFileId))
