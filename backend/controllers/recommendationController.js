...
const generateHealthRecommendations = async (profile) => {
  try {
    // Check cache first
    const cacheKey = `health_v3_${crypto
      .createHash('sha256')
      .update(JSON.stringify(profile.toObject ? profile.toObject() : profile))
      .digest('hex')}`;
...
const generateFirstAidRecommendations = async (profile) => {
  try {
    const cacheKey = `firstaid_${crypto
      .createHash('sha256')
      .update(JSON.stringify(profile.toObject ? profile.toObject() : profile))
      .digest('hex')}`;
    const cached = await RecommendationCache.findOne({ cacheKey: cacheKey });
...
const tryHuggingFaceAPI = async (prompt) => {
  const models = ['microsoft/DialoGPT-medium', 'microsoft/DialoGPT-small', 'gpt2', 'distilgpt2'];
...
const getConciseHealthRecommendations = (profile) => {
  ...
  if (symptoms.length) {
    priorities.push(
      `Discuss current symptoms (${symptoms.slice(0, 3).join(', ')}) with the treating doctor, especially if new or worsening.`
    );
  }
  if (medications.length) {
    priorities.push(
      'Keep an updated medicine list and ask a doctor or pharmacist to review it; do not stop or change medicines without advice.'
    );
  }
  if (allergies.length) {
    priorities.push(
      `Keep these allergies clearly visible to caregivers and clinicians: ${allergies.slice(0, 4).join(', ')}.`
    );
  }
  ...
  if (profile.doctorName && priorities.length < 4) {
    priorities.push(
      `Keep scheduled reviews with ${profile.doctorName} and take an updated symptom and medicine list.`
    );
  }
  ...
  daily.push(
    'Use a simple daily medicine schedule and record missed doses or possible side effects for the next clinical review.'
  );
  if (
    profile.fallRisk ||
    ['walking_aid', 'wheelchair', 'bed_assistance'].includes(profile.mobilityStatus)
  ) {
    daily.push(
      'Reduce fall hazards, use the prescribed mobility aid, and keep frequently used items within easy reach.'
    );
  }
  daily.push(
    'Choose regular balanced meals and fluids that follow existing doctor-provided dietary restrictions.'
  );
  daily.push(
    'Use gentle activity appropriate to current mobility and stop if pain, dizziness, chest discomfort, or unusual breathlessness occurs.'
  );
  daily.push(
    'Maintain a consistent sleep routine and report persistent sleep changes, unusual fatigue, or daytime confusion.'
  );

  urgent.push(
    'Seek urgent medical help for chest pain, severe breathing difficulty, fainting, sudden weakness, new confusion, or uncontrolled bleeding.'
  );
  urgent.push(
    `Emergency contact: ${profile.emergencyContact || 'not provided'}${profile.emergencyPhone ? ` (${profile.emergencyPhone})` : ''}.`
  );
...
const getFallbackHealthRecommendations = (profile) => {
  ...
  const symptoms = profile.currentSymptoms
    ? Array.isArray(profile.currentSymptoms)
  ...
  recommendations.push(
    `• Gender: ${profile.gender || 'Not specified'} | Blood Group: ${profile.bloodGroup || 'Not specified'}`
  );
  recommendations.push(
  ...
  recommendations.push(
    `• Contact: ${profile.contact || 'Not specified'} | Email: ${profile.email || 'Not specified'}`
  );
  recommendations.push('');
  ...
  recommendations.push(
    `• Primary Conditions: ${conditions.length > 0 ? conditions.join(', ') : 'None documented'}`
  );
  ...
  recommendations.push(
    `• 7:30 AM: BREAKFAST - ${getPersonalizedBreakfast(profile, age, conditions)}`
  );
  ...
  recommendations.push(
    '• Herbal: Ashwagandha (500mg) for stress, Turmeric (500mg) for inflammation'
  );
  recommendations.push('');
  ...
  if (medications.length > 0) {
    recommendations.push(
      '• Morning (7:00 AM): ' +
        medications.filter((m) => m.toLowerCase().includes('morning')).join(', ') || 'None'
    );
    recommendations.push(
      '• Afternoon (1:00 PM): ' +
        medications.filter((m) => m.toLowerCase().includes('afternoon')).join(', ') || 'None'
    );
    recommendations.push(
      '• Evening (5:00 PM): ' +
        medications.filter((m) => m.toLowerCase().includes('evening')).join(', ') || 'None'
    );
    recommendations.push(
      '• Bedtime (9:00 PM): ' +
        medications
          .filter((m) => m.toLowerCase().includes('night') || m.toLowerCase().includes('bedtime'))
          .join(', ') || 'None'
    );
  } else {
  ...
  recommendations.push(
    'This is a personalized health recommendation based on provided information.'
  );
...
const getFallbackFirstAidRecommendations = (profile) => {
  ...
  recommendations.push(
    `• Gender: ${profile.gender || 'Not specified'} | Blood Group: ${bloodGroup}`
  );
  recommendations.push(
    `• Height: ${profile.height || 'Not specified'} cm | Weight: ${profile.weight || 'Not specified'} kg`
  );
  recommendations.push(`• BMI: ${bmi || 'Not calculated'} (${bmiCategory})`);
  recommendations.push(
    `• Critical Conditions: ${conditions.length > 0 ? conditions.join(', ') : 'None documented'}`
  );
  ...
  recommendations.push('='.repeat(20));
  recommendations.push(
    `• Primary Emergency Contact: ${profile.emergencyContact || 'Not specified'}`
  );
  ...
  recommendations.push(
    `• Preferred Hospital: ${profile.preferredHospital || 'Nearest Emergency Facility'}`
  );
  ...
  recommendations.push(
    profile.environmentalAllergies ? profile.environmentalAllergies : '• None documented'
  );
  recommendations.push('');
  ...
  recommendations.push(
    '• Cardiac History: ' +
      (hasCondition(conditions, ['heart', 'cardiac']) ? 'Present' : 'None documented')
  );
  recommendations.push(
  ...
  recommendations.push(
    '• Normal Cognitive State: ' + (profile.cognitiveBaseline || 'Alert and oriented')
  );
  ...
  recommendations.push(
    'NEXT REVIEW DUE: ' + new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toLocaleDateString()
  );
  ...
  recommendations.push(
    '• ' + (profile.primaryPhysician ? 'Primary Doctor: ' + profile.primaryPhysician : '')
  );
  recommendations.push(
    '• ' + (profile.emergencyContact ? 'Emergency Contact: ' + profile.emergencyContact : '')
  );
  recommendations.push('');
  recommendations.push('='.repeat(30));
...
const getPersonalizedBreakfast = (profile, age, conditions) => {
  ...
  if (hasCondition(conditions, ['diabetes', 'sugar'])) {
    breakfast =
      '2 besan chilla (made with gram flour) stuffed with mixed vegetables (spinach, carrots, bell peppers) + 1 bowl mixed sprouts (moong, chana) + 1 tsp green chutney';
  } else if (hasCondition(conditions, ['heart', 'hypertension'])) {
    breakfast =
      '1 bowl oats porridge cooked with low-fat milk, topped with 5 almonds (chopped), 2 walnuts, and 1 tsp flaxseeds + 1 boiled egg + 1 seasonal fruit (apple/pear)';
  } else if (hasCondition(conditions, ['arthritis', 'joint'])) {
    breakfast =
      '1 glass turmeric milk (warm milk with ½ tsp turmeric, pinch black pepper) + 2 slices whole wheat toast with 1 tbsp peanut butter + 1 banana';
  } else if (age >= 70) {
    breakfast =
      '4 soaked almonds + 2 whole wheat toast with 1 tsp ghee + 1 bowl curd + 1 glass milk';
  } else {
    breakfast =
      '1 bowl vegetable poha/upma (made with vegetables like peas, carrots, beans) + 1 fruit (seasonal) + 1 glass buttermilk';
  }
...
const getPersonalizedLunch = (profile, age, conditions) => {
  ...
  if (hasCondition(conditions, ['diabetes'])) {
    lunch =
      '2 medium whole wheat chapati + 1 bowl dal (moong/masoor) + 1 bowl seasonal vegetable curry (non-starchy) + large salad (cucumber, tomato, onion) + 1 bowl curd';
  } else if (hasCondition(conditions, ['heart'])) {
    lunch =
      '1 cup brown rice + 1 bowl dal (without excess oil) + 1 bowl steamed vegetables (broccoli, carrots, beans) + salad with lemon dressing + 1 small piece grilled fish (twice weekly)';
  } else if (hasCondition(conditions, ['arthritis'])) {
    lunch =
      '1 cup rice + fish curry (salmon/mackerel rich in omega-3) + 1 bowl green leafy vegetable (spinach/methi) + 1 bowl dal + salad with olive oil dressing';
  } else if (age >= 70) {
    lunch =
      '1 bowl khichdi (rice + moong dal) with ghee + 1 bowl vegetable curry + 1 bowl curd + soft salad';
  } else {
...
const getPersonalizedDinner = (profile, age, conditions) => {
  ...
  if (hasCondition(conditions, ['diabetes'])) {
    dinner =
      '1 multigrain chapati + 1 bowl vegetable curry (bitter gourd, bottle gourd, ridge gourd) + 1 bowl dal + 1 glass buttermilk';
  } else if (hasCondition(conditions, ['heart'])) {
    dinner =
      '1 bowl vegetable soup + 1 chapati + paneer bhurji (low oil) or grilled chicken + steamed vegetables';
  } else if (hasCondition(conditions, ['arthritis'])) {
    dinner =
      '1 bowl moong dal khichdi with ghee + 1 bowl vegetable soup + 1 glass warm milk with turmeric';
  } else if (age >= 70) {
...
const getBloodGroupDiet = (bloodGroup) => {
  ...
  return (
    diets[bloodGroup?.toUpperCase()] ||
    '• No specific blood group diet recommendations available. Follow general healthy eating guidelines.'
  );
};
