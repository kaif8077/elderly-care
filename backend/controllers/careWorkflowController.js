...
const profileCompletion = (profile) => {
  ...
  return {
    percent: Math.round(
      ((Object.keys(required).length - missing.length) / Object.keys(required).length) * 100
    ),
    missing
  };
};
...
exports.getOverview = async (req, res) => {
  const [profile, preferences, careTeam, reminders, qrHistory] = await Promise.all([
    ...
    NotificationPreference.findOne({ userId: req.user.id })
      .select('-telegramChatId -pushSubscriptions')
      .lean(),
    CareRelationship.find({ elderUserId: req.user.id, status: { $ne: 'revoked' } })
      .select('-invitationTokenHash')
      .sort({ createdAt: -1 })
      .lean(),
    Reminder.find({ userId: req.user.id, status: 'active' })
      .sort({ scheduledFor: 1 })
      .limit(20)
      .lean(),
    QrAccessLog.find({ userId: req.user.id })
      .select('event userAgentFamily occurredAt')
      .sort({ occurredAt: -1 })
      .limit(20)
      .lean()
  ]);
...
exports.updatePrivacy = async (req, res) => {
  ...
  Object.entries(req.body.visibility || {}).forEach(([field, level]) => {
    if (PRIVACY_FIELDS.includes(field) && PRIVACY_LEVELS.includes(level))
      updates[`visibilitySettings.${field}`] = level;
  });
  if (req.body.consent && typeof req.body.consent === 'object') {
    ['emergencySharing', 'recommendationGeneration', 'qrAccessLogging'].forEach((field) => {
      if (typeof req.body.consent[field] === 'boolean')
        updates[`consent.${field}`] = req.body.consent[field];
    });
  ...
  return res.json({
    ...
    visibility: profile.visibilitySettings,
    consent: profile.consent
  });
};
...
exports.inviteCareMember = async (req, res) => {
  ...
  if (
    ...
    !['guardian', 'caregiver', 'doctor'].includes(role)
  ) {
  ...
  return res.status(201).json({
    message: 'Care-team invitation created',
    relationship: { ...relationship.toObject(), invitationTokenHash: undefined }
  });
};
...
exports.acceptCareInvitation = async (req, res) => {
  ...
  if (!relationship || relationship.invitedEmail !== user?.email)
    return res.status(404).json({ message: 'Invitation is invalid or belongs to another account' });
...
exports.revokeCareMember = async (req, res) => {
  const relationship = await CareRelationship.findOneAndUpdate(
    ...
    { $set: { status: 'revoked', revokedAt: new Date(), invitationTokenHash: null } },
    { new: true }
  );
...
exports.requestContactVerification = async (req, res) => {
  ...
  if (!contact.email)
    return res.status(400).json({ message: 'Add an email address before verification' });
  const token = crypto.randomBytes(32).toString('base64url');
...
exports.verifyEmergencyContact = async (req, res) => {
  ...
  const profile = await MedicalProfile.findOneAndUpdate(
    {
      emergencyContacts: {
        $elemMatch: { verificationTokenHash: hash, verificationExpiresAt: { $gt: new Date() } }
      }
    },
    {
      $set: {
