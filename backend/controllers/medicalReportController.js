...
exports.list = async (req, res) => {
  try {
    res.json(await listReports({ match: { userId: req.user.id }, page: req.query.page, limit: req.query.limit }));
  } catch (error) {
...
exports.latest = async (req, res) => {
  try {
    const result = await listReports({ match: { userId: req.user.id, isLatest: true, isArchived: false }, page: 1, limit: 5 });
