...
const {
  bucket, detectImageType, removePhoto, storePhoto
} = require('../services/profilePhotoService');

const ownsProfile = (req) => String(req.user.id) === String(req.params.userId);

exports.upload = async (req, res) => {
  if (!ownsProfile(req)) return res.status(403).json({ message: 'You can only update your own photograph' });
  if (!req.file) return res.status(400).json({ message: 'Choose a profile photograph', code: 'PHOTO_REQUIRED' });
  const detected = detectImageType(req.file.buffer);
  if (!detected || detected !== req.file.mimetype) {
    return res.status(400).json({ message: 'The uploaded file is not a valid supported image', code: 'INVALID_PHOTO_CONTENT' });
  }

  try {
    const profile = await MedicalProfile.findOne({ userId: req.user.id }).sort({ createdAt: -1 });
    if (!profile) return res.status(404).json({ message: 'Create a medical profile before uploading a photograph' });
    const previousFileId = profile.profilePhoto?.fileId;
    const stored = await storePhoto({
      buffer: req.file.buffer, contentType: detected, userId: req.user.id
    });
...
exports.remove = async (req, res) => {
  if (!ownsProfile(req)) return res.status(403).json({ message: 'You can only remove your own photograph' });
  try {
    const profile = await MedicalProfile.findOne({ userId: req.user.id }).sort({ createdAt: -1 });
