...
const sectionTitle = (doc, title) => {
  ...
  doc.moveDown(0.25).strokeColor(COLORS.line).lineWidth(1)
...
const labelValue = (doc, label, value) => {
  ensureRoom(doc, 34);
  const y = doc.y;
  doc.fillColor(COLORS.muted).font('Helvetica-Bold').fontSize(9).text(label.toUpperCase(), 48, y, { width: 145 });
  doc.fillColor(COLORS.ink).font('Helvetica').fontSize(10).text(String(value || 'Not provided'), 200, y, { width: 347 });
  doc.y = Math.max(doc.y, y + 24);
};

const generateMedicalReportPdf = (report, { includeInsurance = false } = {}) => new Promise((resolve, reject) => {
  const chunks = [];
  ...
  doc.font('Helvetica-Bold').fontSize(10).text(`VERSION ${report.reportVersion}`, 410, 63, { width: 118, align: 'right' });
  doc.font('Helvetica').fontSize(8).text(report.verificationStatus.replace('_', ' ').toUpperCase(), 410, 84, { width: 118, align: 'right' });
  doc.y = 132;

  doc.fillColor(COLORS.ink).font('Helvetica-Bold').fontSize(20).text(personal.name || 'Unknown person');
  doc.moveDown(0.2).fillColor(COLORS.muted).font('Helvetica').fontSize(9)
    .text(`Generated ${formatDate(report.generatedAt)}  |  Source updated ${formatDate(snapshot.sourceUpdatedAt)}`);

  doc.moveDown(0.8);
  const criticalTop = doc.y;
  doc.roundedRect(48, criticalTop, doc.page.width - 96, 112, 8).fill(COLORS.paleRed);
  doc.fillColor(COLORS.red).font('Helvetica-Bold').fontSize(12).text('CRITICAL EMERGENCY INFORMATION', 64, criticalTop + 15);
  ...
  doc.fillColor(COLORS.ink).font('Helvetica').fontSize(9)
    .text(`Allergies: ${textList(medical.allergies, medical.allergiesOther)}`, 64, criticalTop + 64, { width: 450 })
    .text(`Critical medications: ${textList(medical.medications, medical.medicationsOther)}`, 64, criticalTop + 84, { width: 450 });
  doc.y = criticalTop + 126;

  sectionTitle(doc, 'Personal details');
  labelValue(doc, 'Date of birth', `${formatDate(personal.dob)} (Age ${calculateAge(personal.dob)})`);
  labelValue(doc, 'Gender', personal.gender);
  labelValue(doc, 'Blood group', personal.bloodGroup || 'Unknown');
  labelValue(doc, 'Height / Weight', `${personal.height || '-'} cm / ${personal.weight || '-'} kg`);
  ...
  labelValue(doc, 'Current symptoms', textList(medical.currentSymptoms, medical.currentSymptomsOther));
  ...
  ensureRoom(doc, 90);
  doc.moveDown(1).roundedRect(48, doc.y, doc.page.width - 96, 58, 6).fill(COLORS.paleGreen);
  doc.fillColor(COLORS.ink).font('Helvetica-Bold').fontSize(9)
    .text('Emergency disclaimer', 62, doc.y + 14);
  doc.font('Helvetica').fontSize(8)
    .text('This is an emergency-information summary and is not a replacement for professional medical advice, diagnosis, or treatment.', 62, doc.y + 29, { width: 470 });
  ...
  for (let index = range.start; index < range.start + range.count; index += 1) {
    ...
    doc.strokeColor(COLORS.line).moveTo(48, footerY - 8).lineTo(doc.page.width - 48, footerY - 8).stroke();
    doc.fillColor(COLORS.muted).font('Helvetica').fontSize(7)
      .text('PRIVATE MEDICAL INFORMATION - Share only with authorized persons.', 48, footerY, { width: 390, lineBreak: false })
      .text(`Page ${index + 1} of ${range.count}`, 445, footerY, { width: 102, align: 'right', lineBreak: false });
  }
